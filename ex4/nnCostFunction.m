...
z2 = Theta1*X';     % hidden layer input, (hidden_layer_size * m)
a2 = sigmoid(z2);   % hidden layer output
a2 = a2';

a2 = [ones(m,1), a2];   % m * (hidden_layer_size + 1)

z3 = Theta2*a2';        % output layer input, (num_labels * m)
a3 = sigmoid(z3);
...
for cls = 1:num_labels   % for every output neuron
    ...
    prob_cls = a3(:,cls);% examples' prob of being 'cls'.

    J = J - 1/m*(y_cls'*log(prob_cls)+(1-y_cls)'*log(1-prob_cls));
end

% cost with regularization
reg1 = Theta1(:,2:end).^2; 
reg2 = Theta2(:,2:end).^2;
reg = sum(reg1(:))+sum(reg2(:));
